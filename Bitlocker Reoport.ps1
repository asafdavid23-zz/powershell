...
[CmdletBinding()]
Param (
    [string]$SearchBase = "CN=Computers,DC=in,DC=traffilog,DC=co,DC=il"
)
...
$Computers = Get-ADComputer -Filter {enabled -eq $true} -SearchBase $SearchBase -Properties LastLogonDate, operatingsystem
$Count = 1
...
$Results | Select ComputerName,LastLogonDate,BitLockerPasswordSet,operatingsystem | Sort ComputerName | Export-Csv $ReportPath -NoTypeInformation
